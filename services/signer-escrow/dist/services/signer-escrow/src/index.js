...
async function reconstructPayload(sourceBatchId, escrowBatchId, custodyMode = 'batch_wallet_custody') {
    ...
    const escrowContract = new ethers_1.Contract(ESCROW_ADDRESS, ESCROW_ABI, provider);
    ...
    if (deployedPrincipal === 0 && principalFunded === 0) {
        throw new Error(`Escrow batch ${sourceBatchId} has no deployed principal or funded principal. ` +
    ...
    if (!escrowRole?.exists)
        throw new Error(`ROLE_ESCROW (${ROLE_ID}) not set on InvestmentEscrow`);
    if (Number(escrowRole.status) !== 0)
        throw new Error(`ROLE_ESCROW is not Active (status=${escrowRole.status})`);
    if (escrowRole.signer.toLowerCase() !== SIGNER_ADDRESS) {
        throw new Error(`On-chain ROLE_ESCROW signer (${escrowRole.signer}) does not match this service's key (${SIGNER_ADDRESS}). ` +
...
async function getExecutedTransactionHash(multisig, txIndex) {
    const events = await multisig.queryFilter(multisig.filters.TransactionExecuted(BigInt(txIndex)), -20000).catch(() => []);
    const lastEvent = events[events.length - 1];
    return String(lastEvent?.transactionHash ?? '');
}
// ─── App ───────────────────────────────────────────────────────────────────────
...
app.post('/anchor', async (req, res) => {
    ...
    const value = buildEip712Value(payload);
    const localBindingHash = ethers_1.TypedDataEncoder.hash(domain, BINDING_TYPES, value);
    ...
    const escrowContract = new ethers_1.Contract(ESCROW_ADDRESS, ESCROW_ABI, anchorWallet);
    ...
    const escrowBatchIdHash = (0, ethers_1.keccak256)((0, ethers_1.toUtf8Bytes)(resolvedEscrowBatchId));
    ...
    try {
        const tx = await escrowContract.anchorBatchAuthorityBinding(sourceBatchIdNum, escrowBatchIdHash, localBindingHash, toB32(payload.systemMapHash), toAddr(payload.sourceContractAddress), toAddr(payload.activeEscrowAddress), treasurySignature, escrowSignature);
...
app.post('/attach-allocation', async (req, res) => {
    ...
    try {
        ...
        if (existing?.exists) {
            ...
            if (chainPlanHash.toLowerCase() === String(allocationPlanHash).toLowerCase() &&
        ...
        if (String(keeper).toLowerCase() !== SIGNER_ADDRESS) {
            throw new Error(`Escrow signer service is not InvestmentEscrow keeper. ` +
        ...
        const tx = await escrowContract.attachAllocation(sourceBatchIdNum, String(allocationPlanHash), String(policyContextHash), String(portfolioRegistryVersion));
        const receipt = await tx.wait();
...
app.post('/confirm-deploy-leg', async (req, res) => {
    const { sourceBatchId, escrowBatchId, walletAddress, assetSymbol, amount, destinationAddress, txIndex, chainId, } = req.body ?? {};
    if (!sourceBatchId ||
        !walletAddress ||
        !assetSymbol ||
        !amount ||
    ...
    try {
        ...
        if (normalizeAddress(txTo) !== normalizeAddress(usdcAddress) || txValue !== 0n || txData !== transferData) {
            throw new Error('Batch multisig transaction does not match the approved USDC transfer leg.');
        }
        ...
        const msg = err instanceof Error ? err.message : String(err);
        audit('deploy_leg_confirm_error', { sourceBatchId, walletAddress, destinationAddress, amount, txIndex, error: msg });
