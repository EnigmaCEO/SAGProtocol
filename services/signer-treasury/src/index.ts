...
function escrowBatchUuid(sourceBatchId: string, openedAt: number): string {
  // Keys alphabetical to match sortedJson/canonicalKeccak used by the frontend.
...
async function reconstructPayload(sourceBatchId: string, escrowBatchId: string, custodyMode = 'batch_wallet_custody'): Promise<ReconstructedPayload> {
  ...
  if (!tvRole?.exists) throw new Error(`ROLE_TREASURY_VAULT (${ROLE_ID}) not set on InvestmentEscrow`);
  if (Number(tvRole.status) !== 0) throw new Error(`ROLE_TREASURY_VAULT is not Active (status=${tvRole.status})`);
  if (tvRole.signer.toLowerCase() !== SIGNER_ADDRESS) {
    throw new Error(
      `On-chain ROLE_TREASURY_VAULT signer (${tvRole.signer}) does not match this service's key (${SIGNER_ADDRESS}). ` +
  ...
  const nonce = keccak256(toUtf8Bytes(`sagitta:authority-nonce:${resolvedEscrowBatchId}:${sourceBatchId}`));
...
async function findMatchingTransaction(params: {
  ...
  to: string;
  data: string;
}): Promise<{ txIndex: number; executed: boolean; treasuryConfirmed: boolean } | null> {
  ...
  for (let txIndex = txCount - 1; txIndex >= 0; txIndex -= 1) {
    ...
    if (normalizeAddress(txTo) !== normalizeAddress(to) || txValue !== 0n || txData !== data) continue;
...
app.post('/request-signature', async (req: Request, res: Response) => {
  ...
  try {
    payload = await reconstructPayload(sourceBatchId, escrowBatchId ?? '', resolvedCustodyMode);
  } catch (err: unknown) {
...
app.post('/deploy-leg', async (req: Request, res: Response) => {
  const {
    sourceBatchId,
    escrowBatchId,
    walletAddress,
  ...
  try {
    ...
    let resolvedEscrowBatchId: string;
    if (typeof escrowBatchId === 'string' && escrowBatchId) {
      resolvedEscrowBatchId = escrowBatchId;
    } else {
      const treasury = new Contract(TREASURY_ADDRESS, TREASURY_ABI, provider);
      const b = await treasury.getTreasuryBatch(BigInt(sourceBatchId)).catch(() => null);
      resolvedEscrowBatchId = escrowBatchUuid(String(sourceBatchId), Number(b?.openedAt ?? 0));
    }
    res.json({
      role: ROLE,
      signerAddress: SIGNER_ADDRESS,
    ...
    const msg = err instanceof Error ? err.message : String(err);
    audit('deploy_leg_error', { sourceBatchId, assetSymbol: normalizedAssetSymbol, walletAddress, destinationAddress, amount, error: msg });
